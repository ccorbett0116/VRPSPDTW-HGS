...
void Population::generatePopulation()
{
	...
	for (int i = 0; i < 4*params.ap.mu && (i == 0 || params.ap.timeLimit == 0 || (double)(clock() - params.startTime) / (double)CLOCKS_PER_SEC < params.ap.timeLimit) ; i++)
	{
		...
		std::cout << "Max population size: " << params.ap.mu + params.ap.lambda << " individuals.            \r" << std::flush;
	}
}
...
bool Population::addIndividual(const Individual & indiv, bool updateFeasible)
{
	if (updateFeasible)
	{
		...
		listFeasibilityTimeWarp.push_back(indiv.eval.timeWarp < MY_EPSILON);
...
void Population::managePenalties()
{
	...
	// NEW: Time warp penalty management
	// Track feasibility based on time warp (no time warp = feasible)
	double fractionNoTimeWarp = (double)std::count(listFeasibilityTimeWarp.begin(), listFeasibilityTimeWarp.end(), true) / (double)listFeasibilityTimeWarp.size();
	if (fractionNoTimeWarp < params.ap.targetFeasible - 0.05 && params.penaltyMultiplier < 100000.)
		params.penaltyMultiplier = std::min<double>(params.penaltyMultiplier * params.ap.penaltyIncrease, 100000.);
	else if (fractionNoTimeWarp > params.ap.targetFeasible + 0.05 && params.penaltyMultiplier > 0.1)
		params.penaltyMultiplier = std::max<double>(params.penaltyMultiplier * params.ap.penaltyDecrease, 0.1);
	...
	for (int i = 0; i < (int)infeasibleSubpop.size(); i++)
		...
		+ params.penaltyMultiplier * infeasibleSubpop[i]->eval.timeWarp; // NEW: Add time warp penalty
...
Population::Population(Params & params, Split & split, LocalSearch & localSearch) : params(params), split(split), localSearch(localSearch), bestSolutionRestart(params), bestSolutionOverall(params)
{
	...
	listFeasibilityTimeWarp = std::list<bool>(params.ap.nbIterPenaltyManagement, true);
}
