...
static Solution *prepare_solution(Population &population, Params &params)
{
    ...
    int max_nbVeh, const AlgorithmParameters *ap, char verbose)
{
    ...
    try {
        ...
        std::vector<double> pickups(n, 0.0); // <-- C API has no pickups input; assume 0

        // Distance/time matrices (Euclidean) — keep legacy behavior
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                ...
                if (isRoundingInteger) d = std::round(d);
                distance_matrix[i][j] = d;
                time_matrix[i][j]     = d; // use distance as travel time by default
            }
        }

        // Default wide-open time windows for backward compatibility
        std::vector<double> ready_time(n, 0.0);
        std::vector<double> due_time(n, 1e6);

        Params params(
            x_coords, y_coords,
            distance_matrix, time_matrix,
            service_time, demands, pickups,     // <-- pass zero pickups
            vehicleCapacity, durationLimit,
            ready_time, due_time,
            0.0, 0.0,                           // dispatchingCost, unitCost (not used in this C API)
            max_nbVeh,
            (isDurationConstraint != 0),        // char -> bool
            (verbose != 0),                     // char -> bool
            *ap
        );
    ...
    int max_nbVeh, const AlgorithmParameters *ap, char verbose)
{
    ...
    try {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                double d = dist_mtx[n * i + j];
                distance_matrix[i][j] = d;
                time_matrix[i][j]     = d; // use provided dist as time by default
            }
        }
